...
import CompletedBuyer from "../components/CompletedBuyer";
...
useEffect(() => {
  const fetchLedger = async () => {
    try {
      ...
      const response = await axios.get(
        `http://10.0.2.2/database/fetchLedgerBuyer.php?buyer_id=${user.user_id}`
      );
  ...
  return (
    <View style={styles.container}>
      ...
      <Pressable style={styles.halfButton} onPress={() => router.push('/productStateBuyerReceive')}>
      ...
      <View style={styles.bodySection}>
        <FlatList
         ...
         renderItem={({ item }) => <CompletedBuyer item={item} />}
